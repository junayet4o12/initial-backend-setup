...
import { sendEmail, sendLinkViaMail, sendOtpViaMail } from '../../utils/sendMail';
...
const loginWithOtpFromDB = async (res: Response, payload: {
  email: string;
  password: string;
}) => {
...
const loginFromDB = async (res: Response, payload: {
  email: string;
  password: string;
}) => {
  const userData = await insecurePrisma.user.findUniqueOrThrow({
